...
def set_k_p(interface, motor, k_p):
...
if __name__ == '__main__':
    ...
    set_k_p(interface, motors[0], 850)
    k_ps = [750 + x * 50.0 for x in range(0, 4)]

    for k_p in k_ps:
        set_k_p(interface, motors[1], k_p)
        print('Set k_p to {}'.format(k_p))
        interface.startLogging(LOG_PATH + 'k_p_d_{}'.format(int(k_p * 100)))
